...
Per-repo config consumed by the workflow skills, written for this repo by upstream's `/setup-matt-pocock-skills`. Config dir: `.agents/`.

### Workflow backend

**Backend:** notion. The recipe mapping each tracker operation (publish, fetch, list, comment, transition state, attach an agent brief) to its Notion MCP call lives in [`.agents/issue-tracker.md`](./.agents/issue-tracker.md). The Issues / PRDs / Handoffs database IDs and property mappings live in the gitignored `.agents/workflow-config.md`, maintained by hand.
